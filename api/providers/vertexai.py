...
class VertexAIProvider(BaseModel):
    ...
    def __init__(self, config: RouteConfig) -> None:
        ...
        if config.model.config is None or not isinstance(config.model.config, VertexAIConfig):
            # Should be unreachable
            raise ValueError(
                "Invalid config type {config.model.config}"
            )
        self.vertexai_config: VertexAIConfig = config.model.config
    ...
    async def chat(self, data) -> dict:
        ...
        if model_class in {TextGenerationModel, CodeGenerationModel}:
            ...
            response = self.predict(model, input_arg_name,
                            data.chat_input, data.is_stream, **kwargs)
        else:
            model = model_class.from_pretrained(data.model_name)
            response = self.chat_predict(model, data.chat_input,
